...
4. **还款管理**：先在“资料待办”确认债权人、债务人、协议总额和分期计划，再在“协议履约”查看累计回款、剩余金额、逾期和提醒。每份协议按具体期次建立 D-7、到期日和逾期 D+3 提醒，发送到协议来源群并默认 `@` 协议原消息发送人；后台可改选当前群成员，并可对最早未还期次人工“立即催还款”。同协议、同一期、同一天只允许一次人工催款。回款说明文字只用于关联相邻截图；回款凭证必须选择同群的具体协议，人工批准后才计入累计回款、取消已足额期次提醒并更新该协议原有的金山行；全部结清后停止该协议所有提醒。
