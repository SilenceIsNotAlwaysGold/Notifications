...
def list_document_sync_logs(
    ...
    total = _total if len(items) == fetched_count else len(items)
    data = DocumentSyncLogListOut(total=total, items=[DocumentSyncLogOut.model_validate(item) for item in items])
