            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "群聊上下文包含相邻消息、附件 OCR 摘要、群资料和历史关联案件，可用于补充案号、当事人和材料用途。"
                        "历史关联案件列表仅是候选范围，即使列表中只有一个案件，也绝不能据此认定当前材料属于该案件；"
                        ...
                        "聊天截图中如果展示已签署的仲裁协议、还款方案、明确还款日期和金额，应识别为 repayment_agreement；"
                        "群聊标题中的‘商家简称-自然人姓名’只能作为债权人简称和债务人候选，必须设置 requires_review=true 等待映射正式主体。"
                        "聊天中‘怎么没付款’、‘记得准时还款’、催促回应、到期未履行等内容属于逾期或催收证据，"
                        "绝不能识别为 payment_notice 或 payment_screenshot；payment_screenshot 只表示真实付款凭证或明确确认已付款。"
                        "截图同时包含还款约定和逾期对话时，应优先抽取还款方案，并在 review_reasons 中标明存在逾期证据。"
                        ...
                        "不得把‘经传票传唤’等正文固定用语识别为当事人。"
                        "不得把其他案件的信息套用到当前材料。群内案件数量会持续增加，必须依据明确案号或紧邻对话建立关联。"
