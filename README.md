...
3. 每条结果都经过自动化闸门。结构化置信度达到自动阈值、关键字段完整、关联唯一且没有冲突时，系统直接执行；任一条件不满足时进入对应人工待办，确认前不记账、不提醒、不写金山。
4. 缴费通知不要求案件匹配，直接以原消息所在群和发送人为跟进对象；高置信度通知自动开始 30/90 分钟提醒，低置信度通知由人工确认后从确认时刻开始计时。收到可唯一关联的明确付款文字或凭证后结束。
5. 执行文书、开庭传票和还款资料在高置信度时通过事务型 outbox 自动上传原文件并写入对应金山子表；低置信度资料由人工修正或确认后走相同 outbox，保存工作表、行号、映射版本和读回结果。
6. 商家消息仅在确实需要回答且判断置信度足够时自动跟踪；模糊语境进入人工判断，普通聊天、表情、结束语和平台自己发送的提醒不会创建新任务。
...
- 可配置的自动写入、自动提醒和 AI 最低置信度阈值
- 执行文书、开庭传票、还款协议、回款凭证与模糊消息的独立人工工作区
...
LEGAL_AUTO_WRITE_MIN_CONFIDENCE=0.90
...
自动阈值只是必要条件，不是唯一条件。缺少当事人、金额、法院、时间或分期计划，付款/回款无法唯一关联，AI 回退到规则，重复文件或结果存在冲突时，即使分数达到阈值也必须人工确认。每次判断会写入 `metadata.automation_decision`，记录动作、结果、分数、阈值、判断来源和拦截原因。
